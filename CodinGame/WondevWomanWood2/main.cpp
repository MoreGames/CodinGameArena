...
void setCellLevel(state& s, int index, int level) {
	s.board[index][0] = level;
}
void updateCell(state& s, int x, int y, char c) {
	int index = y*s.size + x;
	...
	if (c == '.') {
		...
		if (level == 4) {
			...
			setCellLevel(s, index, c-48);
		}
		//fprintf(stderr, "update index:%2d %2d,%2d level:%2d\n", index, x, y, (c-48));
	}
}
...
int highestNeighbourCell(state& s, int index, int maxLevel = 3) {
	int highLevel = -1;
	int highIndex = -1;
	for (size_t i = 2; i < 10; i++) {
		int n = s.board[index][i];
		if (n != -1) {
			...
			for (size_t j = 0; j < s.unitsPerPlayer; j++) {
				int ui = s.other[j][0] + s.size*s.other[j][1];
				if (n == ui) {
					otherPlayer = true;
				}
			}
...
int main() {
    ...
    while (1) {
		...
		int posMe = base.me[0][0] + base.size*base.me[0][1];
		int maxLevel = base.board[posMe][0]+1;
		maxLevel = maxLevel>3?3:maxLevel;
		int newPos = highestNeighbourCell(base, posMe, maxLevel);
		
		string moveDir = direction(posMe, newPos, size);
		int levelPos = highestNeighbourCell(base, newPos, 2);
		string levelDir = direction(newPos, levelPos, size);
		
		//fprintf(stderr, "me:%2d new:%2d level:%2d %s %s\n", posMe, newPos, levelPos, moveDir.c_str(), levelDir.c_str());
		
		// find any possible neighbour, without level up to 4
		if (levelPos == -1) {
			for (size_t i = 2; i < 10; i++) {
				newPos = base.board[posMe][i];
				if (newPos == -1) continue;
				
				bool otherPlayer = false;
				for (size_t j = 0; j < base.unitsPerPlayer; j++) {
					int ui = base.other[j][0] + base.size*base.other[j][1];
					if (newPos == ui) {
						otherPlayer = true;
					}
				}
				...
				moveDir = direction(posMe, newPos, size);
				levelPos = highestNeighbourCell(base, newPos, 2);
				levelDir = direction(newPos, levelPos, size);
		...
		if (levelPos == -1) {
			for (size_t i = 2; i < 10; i++) {
				...
				moveDir = direction(posMe, newPos, size);
				levelPos = highestNeighbourCell(base, newPos);
				levelDir = direction(newPos, levelPos, size);
		...
		// find any possible neighbour
		
		cout << "MOVE&BUILD 0 " << moveDir <<  " " << levelDir << endl;
